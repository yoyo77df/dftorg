...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [
      { title: "Sign in or Create Account — FFBPL MATCH" },
      { name: "description", content: "Sign in to FFBPL MATCH or create your gamer account to join real-money esports tournaments." },
      { property: "og:title", content: "Sign in or Create Account — FFBPL MATCH" },
      { property: "og:description", content: "Sign in or create your FFBPL MATCH gamer account to start competing." },
...
function AuthPage() {
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success("Account created! Welcome to FFBPL MATCH…");
      await navigate({ to: "/dashboard", replace: true });
    } catch (err: any) {
      toast.error(mapFirebaseError(err?.code));
    } finally {
    ...
    <div className="container mx-auto flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="glass rounded-2xl p-8 neon-border">
          <h1 className="text-2xl font-bold">Welcome to <span className="text-gradient">FFBPL MATCH</span></h1>
